...
function encrypt(email, password, api_id, local_start_date, local_end_date) {
  var XMLString = '<root><method>get_device_on_change_telemetry</method><user><email>' + email + 
      "</email><password>" + password + "</password></user><api_id>" + api_id + "</api_id><local_start_date>" + local_start_date + 
...
function get_device_telemetry_xml(req, res) {
  ...
  if (validate_email(req.body.root.user.email) !== 'valid'){
    ...
    function (error, stdout, stderr) {
      ...
      // Input validation
      if (response && error !== null) {
        alert_slack('API - get_device_telemetry exec error: ' + err);
        res.status(503);
        res.send('<error>Service temporarily unavailable</error>');
      } else if (response && response.message == 'invalid') {
