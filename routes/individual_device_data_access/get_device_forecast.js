...
function log(err, alert_string){
  console.log(err);
  alert_slack('API get_device_forecast ' + alert_string);
}
...
function encrypt(email, password, api_id, local_date) {
  var XMLString = '<root><method>get_device_forecast</method><email>' + email + 
      "</email><password>" + password + "</password><api_id>" + api_id + "</api_id><local_date>" + local_date + 
      "</local_date></root>";
...
function get_device_telemetry_json(req, res) {
  if (validate_email(req.body.root.user.email) !== 'valid'){
    ...
    res.status(400).json({
      error: "local_date: " + validate_date(req.body.root.local_date)
    });
  } else if (new Date(req.body.root.local_date) - new Date() > 604800000) {
    res.status(400).json({
      error: "Date cannot exceed 7 days in advance"
    });
    ...
    var encryption = encrypt(req.body.root.user.email, req.body.root.user.password, req.body.root.api_id, req.body.root.local_date);
    ...
    function (error, stdout, stderr) {
      ...
      } else if (response && response.message == 'data unavailable') {
        log(null, 'user: ' + req.body.root.user.email + ' data not available for requested date')
        res.status(409).json({
...
function get_device_telemetry_xml(req, res) {
  ...
  if (validate_email(req.body.root.user.email) !== 'valid'){
    ...
    res.status(400)
    res.send('<error>local_date: ' + validate_date(req.body.root.local_date) + '</error>');
  } else if (new Date(req.body.root.local_date) - new Date() > 604800000) {
    res.status(400)
    res.send('<error>Date cannot exceed 7 days in advance</error>');
  } else if (has_whitespace(req.body.root.user.email) || has_whitespace(req.body.root.user.password) || has_whitespace(req.body.root.api_id)){
    res.status(400);
    ...
    function (error, stdout, stderr) {
      ...
      if (response && error !== null) {
        ...
        res.status(409);
        res.send('<error>Data not available for the requested date</error>');
      } else if (response && response.message == 'device unavailable') {
        res.status(409);
...
router.post('/', (req, res, next) => {
  ...
  if (response_format == 'xml'){
    if (!req.body.root || !req.body.root.user || !req.body.root.user.email || !req.body.root.user.password || !req.body.root.api_id || !req.body.root.local_date) {
      res.status(400);
