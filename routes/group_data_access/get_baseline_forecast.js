...
function log(err, alert_string){
  console.log(err);
  alert_slack(' API get_baseline_forecast ' + alert_string);
}
...
function encrypt(email, password, date, group) {
  var XMLString = '<root><method>get_baseline_forecast</method><email>' + email + 
...
// function get_splits(file_name, res, baseline, response_format) {
//   fs.readFile('splits/' + file_name + '.json', 'utf8', function read(err, data) {
//     var split;
//     if(err){
...
//       var length;
//       try {
//         length = baseline.Forecast.length;
...
//         var KW = baseline.Forecast[index-1].Forecast_KW;
//         baseline.Forecast[index-1].Forecast_KW = KW * split[0];
//         baseline.Forecast.splice(index, 0, {
//           "Timeslot": baseline.Forecast[index-1].Timeslot.substring(0, 3) + ("0" + (parseInt(baseline.Forecast[index-1].Timeslot.substring(3, 5)) + 5)).slice(-2),
//           "Forecast_KW": KW * split[1]
//         });
//         index++;
//         baseline.Forecast.splice(index, 0, {
//           "Timeslot": baseline.Forecast[index-1].Timeslot.substring(0, 3) + ("0" + (parseInt(baseline.Forecast[index-2].Timeslot.substring(3, 5)) + 10)).slice(-2),
//           "Forecast_KW": KW * split[2]
//         });
//         index+=2;
//       }
//       if (response_format == 'xml') {
...
function get_baseline_forecast_json(req, res) {
  var today = new Date();
  // var found = false;
  // if(!projects){
  //   res.status(503).json({
  ...
  if(req.body.root.group){
    if (!req.body.root.user.email || !req.body.root.user.password || !req.body.root.local_date || !req.body.root.group){
      ...
      res.status(400).json({
        ...
        has_whitespace(req.body.root.local_date)){
      res.status(400).json({
      ...
      //   var localDate = new time.Date(req.body.root.local_date + 'T00:00', projects[i].timezone);
      ...
      //       found = true;
            ...
            var encryption = encrypt(req.body.root.user.email, req.body.root.user.password, req.body.root.local_date, req.body.root.group);
    ...
    if (!req.body.root.user.email || !req.body.root.user.password || !req.body.root.local_date || !req.body.root.group_id){
    ...
    } else if (typeof req.body.root.user.password == "object" || typeof req.body.root.group_id == "object") { //  duplicate passwords, groups
      res.status(400).json({
        error: 'Duplicate fields'
      });
    } else if (validate_email(req.body.root.user.email) !== 'valid'){
      ...
      //       found = true;
            // console.log(projects[i].dateEnded);
            var encryption = encrypt(req.body.root.user.email, req.body.root.user.password, req.body.root.local_date, req.body.root.group_id);
            exec('sh /var/www/html/api.shiftedenergy.com/scripts/api_call.sh' + " " + encryption,
            function (error, stdout, stderr) {
              var baseline;
              var decryption;
              ...
              if (baseline && error !== null) {
                ...
                log(null, 'user: ' + req.body.root.user.email + ' forecast unavailable for group ' + req.body.root.group_id)
                // on forecast unavailable for date
                res.status(409).json({
                  message: 'Forecast unavailable'
                });
...
function get_baseline_forecast_xml(req, res) {
  var today = new Date();
  // var found = false;
  ...
  if(req.body.root.group){
    if (!req.body.root.user.email || !req.body.root.user.password || !req.body.root.local_date || !req.body.root.group){
      ...
      res.send('<error>' + validate_date(req.body.root.local_date) + '</error>');
        ...
        has_whitespace(req.body.root.local_date) || has_whitespace(req.body.root.group)){
      res.status(400);
    ...
    if (!req.body.root.user.email || !req.body.root.user.password || !req.body.root.local_date || !req.body.root.group_id){
      ...
      res.send('<error>' + validate_date(req.body.root.local_date) + '</error>');
        ...
        has_whitespace(req.body.root.local_date) || has_whitespace(req.body.root.group_id)){
      res.status(400);
      res.send('<error>Remove all whitespace from body</error>');
    } else if (!String(req.body.root.group_id).match("^[0-9]+$") || !Number.isInteger(parseInt(req.body.root.group_id))) {
      res.status(400);
      res.send('<error>Invalid group_id</error>');
    } else {
      // for (var i = 0; i < projects.length; i++) {
      //   var localDate = new time.Date(req.body.root.local_date + 'T00:00', projects[i].timezone);
      //   if (projects[i].projectID == req.body.root.group) {
  ...
  //     projects = JSON.parse(data);
      ...
      if (response_format == 'xml'){
        if (!req.body.root || !req.body.root.user) {
          ...
          get_baseline_forecast_xml(req, res);
        }
        ...
        if (!req.body.root || !req.body.root.user) {
          res.status(400).json({
            ...
            get_baseline_forecast_json(req, res);
        }
      } else {
        res.status(400).json({
