...
function encrypt(email, password, group) {
  var XMLString = '<root><method>get_controllers_in_group</method><email>' + email + 
...
function get_controllers_in_group_json(req, res) {
  var found = false;
  if (!req.body.root.user.email || !req.body.root.user.password || !req.body.root.group){
    res.status(400).json({
    ...
    //     found = true;
        var encryption = encrypt(req.body.root.user.email, req.body.root.user.password, req.body.root.group);
        ...
        function (error, stdout, stderr) {
          ...
          if (!res.headersSent && controllers && error !== null) {
            log('exec error: ' + error, 'user: ' + req.body.root.user.email + ' error calling WebDNA');
            res.status(503).json({
              error: 'Service temporarily unavailable'
            });
          } else if (!res.headersSent && controllers && controllers.message == 'invalid') {
            log(null, 'user: ' + req.body.root.user.email + ' authentication invalid')
            res.status(401).json({
              error: 'Authentication invalid'
            });
          } else if (!res.headersSent && controllers && controllers.message == 'Group unavailable') {
            log(null, 'user: ' + req.body.root.user.email + ' group unavailable: ' + req.body.root.group)
            res.status(409).json({
              message: 'Group unavailable'
            });
          } else if (!res.headersSent && controllers && controllers.message == 'Device list unavailable') {
            log(null, 'user: ' + req.body.root.user.email + ' device list unavailable: ' + req.body.root.group)
            res.status(409).json({
              message: 'Device list unavailable'
            });
          } else if (!res.headersSent && controllers && controllers.message == 'Not Authorized') {
            ...
            log(null, 'user: ' + req.body.root.user.email + '  no response from WebDNA')
            res.status(503).json({
              error: 'Service temporarily unavailable'
            });
          }
