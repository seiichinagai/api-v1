...
function get_group_four_day_baseline_forecast_json(req, res, projects) {
  var today = new Date();
  var found = false;
  if(!projects){
    res.status(503).json({
  ...
  } else if (typeof req.body.root.user.password == "object") { //  duplicate passwords, groups
    res.status(400).json({
...
function get_group_four_day_baseline_forecast_xml(req, res, projects) {
  var today = new Date();
  var found = false;
...
/*
  Look at get_sums from get_multiple_group_load_shift_forecast
  Need to loop through groups and retrieve 4 day forecast and aggregate them
*/
...
router.post('/', (req, res, next) => {
  ...
  if (response_format == 'xml'){
    if (!req.body.root.user.email || !req.body.root.user.password || !req.body.root.local_first_data_date || !req.body.root.groups) {
      res.status(400);
