...
function encrypt(email, password, group, event_date) {
  var XMLString = '<root><method>get_ffr_event_details</method><email>' + email + "</email><password>" + password + 
  "</password><group>" + group + "</group><event_date>" + event_date + "</event_date></root>";
...
function get_ffr_event_details_json(req, res, projects) {
  var found = false;
  if(!projects){
    res.status(503).json({
  ...
  } else if (typeof req.body.root.user.password == "object" || typeof req.body.root.user.group == "object" || typeof req.body.root.event_date == "object") {
  ...
  } else if (validate_date(req.body.root.event_date) !== 'valid') {
    res.status(400).json({
      error: "Start date: " + validate_date(req.body.root.event_date)
    });
    ...
    for (var i = 0; i < projects.length; i++) {
      if (projects[i].projectID == req.body.root.group && !res.headersSent) {
        found = true;
        var encryption = encrypt(req.body.root.user.email, req.body.root.user.password, req.body.root.group, req.body.root.event_date);
...
function get_ffr_event_details_xml(req, res, projects) {
  var found = false;
  ...
  if (!projects){ // executed if call is made to server just after server restart, before projects is loaded
  ...
  } else if (typeof req.body.root.user.password == "object" || typeof req.body.root.group == "object" || typeof req.body.root.event_date == "object") {
    ...
    res.status(400)
    res.send('<error>Start date: ' + validate_date(req.body.root.event_date) + '</error>');
...
router.post('/', (req, res, next) => {
  ...
  if (response_format == 'xml'){
    if (!req.body.root || !req.body.root.user || !req.body.root.user.email || !req.body.root.user.password || !req.body.root.group || !req.body.root.event_date) {
      res.status(400);
      ...
      fs.readFile('/var/www/html/api.shiftedenergy.com/projects.db', 'utf8', function read(err, data) {
        ...
        try {
          ...
          get_ffr_event_details_xml(req, res, projects);
        } catch (err) {
          console.log(err);
        }
      });
    ...
    if (!req.body.root || !req.body.root.user || !req.body.root.user.email || !req.body.root.user.password || !req.body.root.group || !req.body.root.event_date) {
      ...
      fs.readFile('/var/www/html/api.shiftedenergy.com/projects.db', 'utf8', function read(err, data) {
        ...
        try {
          projects = JSON.parse(data);
          get_ffr_event_details_json(req, res, projects);
        } catch (err) {
          console.log(err);
        }
      });
