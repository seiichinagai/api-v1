...
function encrypt(email, password, group) {
  var XMLString = '<root><method>get_ffr_group_settings</method><email>' + email + 
...
function get_ffr_settings_json(req, res, projects) {
  var found = false;
  if(!projects){
    res.status(503).json({
...
function get_ffr_settings_xml(req, res, projects) {
  var found = false;
  ...
  if (response_format == 'xml'){
    if (!req.body.root || !req.body.root.user || !req.body.root.user.email || !req.body.root.user.password || !req.body.root.group) {
      ...
      fs.readFile('/var/www/html/api.shiftedenergy.com/projects.db', 'utf8', function read(err, data) {
        ...
        try {
          ...
          get_ffr_settings_xml(req, res, projects);
        } catch (err) {
          console.log(err);
        }
      });
    ...
    if (!req.body.root || !req.body.root.user || !req.body.root.user.email || !req.body.root.user.password || !req.body.root.group) {
      ...
      fs.readFile('/var/www/html/api.shiftedenergy.com/projects.db', 'utf8', function read(err, data) {
        ...
        try {
          projects = JSON.parse(data);
          get_ffr_settings_json(req, res, projects);
        } catch (err) {
          console.log(err);
        }
      });
