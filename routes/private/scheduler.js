  ...
  if (login == "shifted_api" && password == "k&iAmV$Evd"){
    try {
      if(!res.headerSent && !req.files) {
        ...
        fs.writeFile('/var/www/html/api.shiftedenergy.com/scheduler/uploads/' + file.data.name, file.data.data.toString('utf-8'), function(err){
          if(err){console.log(err);}
          // console.log(file.data.data.toString('utf-8'))
          var run_scheduler_command = '/var/www/html/api.shiftedenergy.com/scheduler/run_scheduler_v2.sh ' + file.data.name;
