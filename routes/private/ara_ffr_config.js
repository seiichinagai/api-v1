...
router.post('/', (req, res, next) => {
  ...
  if (login == "webDNA" && password == "de0xyr!bonucleic4c!d"){
    console.log(new Date().toISOString() + ' WebDNA calling Ara FFR Config.')
    try {
      fs.readFile('/var/www/html/api.shiftedenergy.com/apricity/access_token.db','utf8',function(token_err,data){
        ...
        for (var i = 0; i < devices.length; i++){
          body.commands.push({
              ...
              "tripThreshold": req.body.tripThreshold,
              "returnThreshold": req.body.returnThreshold,
              "badCycles": req.body.badCycles,
              "goodCycles": req.body.goodCycles,
              "randomTurnOn": req.body.randomTurnOn,
              "maxDuration": 1800
            })
        }
